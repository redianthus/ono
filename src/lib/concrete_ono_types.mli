(** Shared types for the concrete ono runtime.
    ...
    Groups the externs that both the text and GUI back-ends must implement, so
    they can be assembled into a uniform set of primitives. *)
...
(** Concrete back-end interface: the trio of primitives that every output mode
    (text, GUI, …) must provide. *)
