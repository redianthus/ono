(** Symbolic execution driver for ono.
    ...
    against the symbolic ono runtime, then explores all reachable paths with the
    Owi symbolic interpreter (Z3 backed, FIFO exploration). *)

val run :
  source_file:Fpath.t ->
  no_stop_at_failure:bool ->
  (unit, Owi.Result.err) result
(** [run ~source_file ~no_stop_at_failure] symbolically executes the WAT module
    at [source_file].
