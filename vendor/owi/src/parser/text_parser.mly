...
%token ALIGN ASSERT_EXHAUSTION ASSERT_INVALID ASSERT_MALFORMED ASSERT_RETURN ASSERT_TRAP ASSERT_UNLINKABLE 
%token BINARY BLOCK BR BR_IF BR_TABLE
%token CALL CALL_INDIRECT CALL_REF
%token DATA DATA_DROP DECLARE DROP
%token ELEM ELEM_DROP ELSE END EOF EQUAL EXPORT EXTERN EXTERN_EXTERNALIZE EXTERN_INTERNALIZE EXTERN_REF
...
%token NAN_ARITH NAN_CANON NOP NULL
%token OFFSET
%token PARAM
%token QUOTE
%token REF REF_EXTERN REF_FUNC REF_HOST REF_IS_NULL REF_NULL REGISTER RESULT RETURN RETURN_CALL RETURN_CALL_INDIRECT RETURN_CALL_REF RPAR
%token SELECT START
